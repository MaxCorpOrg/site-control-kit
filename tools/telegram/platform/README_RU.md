...
- в первом режиме умеет загрузить `.txt` / `.csv` / `.json` список username прямо с компьютера и реально добавлять эти username в контакты выбранного сверху Telegram Desktop portable-профиля;
