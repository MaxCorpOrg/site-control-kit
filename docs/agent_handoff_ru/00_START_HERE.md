# Передача контекста: с чего начать
...
В корне репозитория теперь есть ещё более короткая входная точка:
- `START_HERE_AGENT_RU.md`

Новый агент должен упереться сначала в неё, чтобы не начать работу "просто с чтения всего подряд", а сразу определить последнюю подтверждённую точку проекта и продолжать именно от неё.
