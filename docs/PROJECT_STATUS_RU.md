...
5. Для Telegram control center: вывести в отдельный маленький блок явное предупреждение, что `Непрерывно до Стопа` отключает дальнейшее автоматическое чередование и удерживает панель в одном session-run.
6. Для Telegram control center: проверить глазами на реальном операторском сценарии, хватает ли нового summary-блока без прокрутки, или нужно ещё сильнее укрупнить шрифт и сократить технические поля.
7. Снизить runtime-затраты discovery относительно deep, чтобы multi-peer deep чаще успевал проходить следующий слой visible peer.
8. Поднять приоритеты deep-target'ов: раньше брать тех peer, у кого вероятность успешного `Mention` выше.
9. Разделить browser capability/runtime compatibility и Telegram export concerns в отдельные модули/слои.
10. Отделить понятие `best-known latest` от `most-recent run` в UI и документации, если пользователю важно видеть именно последний прогон как основной артефакт.
11. Декомпозировать `export_telegram_members_non_pii.py` на модули.
