...
- Найден и закрыт важный functional gap:
  - UI, handoff и docs уже говорили, что `public_phones` V1 умеет `chat about`, `pinned/history` text и `public bio/about`;
  - фактический helper `scripts/telegram_tdata_helper.py` до этого реально собирал только `user about`.
- Что исправлено:
  - [scripts/telegram_tdata_helper.py](../scripts/telegram_tdata_helper.py) теперь действительно собирает номера из:
    - `chat about`
    - `pinned/history` message text
    - `public bio/about`
  - [scripts/export_telegram_members_non_pii.py](../scripts/export_telegram_members_non_pii.py) теперь пишет корректное пояснение в итоговый markdown;
  - regression добавлен в:
    - [tests/test_telegram_tdata_helper.py](../tests/test_telegram_tdata_helper.py)
    - [tests/test_telegram_export_runtime.py](../tests/test_telegram_export_runtime.py)
- Verify этого pass:
...
- Live smoke по текущему AK2 source:
  - direct helper-run через обычный `python3` на этой машине не годится:
    - `Missing opentele dependency. Run this helper via the collector venv.`
  - direct helper-run через `/home/max/telegram-api-collector/.venv/bin/python` уже сохранён:
...
- Что важно:
  - `default_user` не менялся;
  - использовался только текущий GUI path, без возврата к старому bridge-heavy route;
  - во время первого full-history run найден реальный runtime bug: `export-public-phones` использовал `TELEGRAM_TDATA_LIST_TIMEOUT_SEC`, поэтому падал через `30s` до первого meaningful progress.
- Что исправлено:
  - [scripts/telegram_gui/app.py](../scripts/telegram_gui/app.py) и [scripts/telegram_gui/ui/window.py](../scripts/telegram_gui/ui/window.py) теперь отправляют все `export-*` helper-команды в export-timeout branch;
  - [tests/test_telegram_members_export_gui.py](../tests/test_telegram_members_export_gui.py) теперь отдельно держит это правилом.
- Verify после фикса:
...
- Артефакты неудачного первого запуска:
  - `/home/max/Документы/ак2/живой_тест_номеров/ak2_cosmetology_public_phones_full_history_summary_20260602T110234Z.json`
  - `/home/max/Документы/ак2/живой_тест_номеров/ak2_cosmetology_public_phones_full_history_summary_20260602T110234Z.md`
  - `/home/max/.site-control-kit/telegram_workspace/logs/gui_ak2_cosmetology_full_history_phones_20260602T110234Z.log`
  - failure text: `tdata helper timed out after 30s: export-public-phones`
- Повторный live-run с временным env-workaround `TELEGRAM_TDATA_LIST_TIMEOUT_SEC=21600` и `TELEGRAM_TDATA_PROGRESS_EVERY=100` дал:
  - `Косметология`:
    ...
    - файлы:
      - `/home/max/Документы/ак2/живой_тест_номеров/ak2_cosmetology_01_kosmetologi_chat_full_history_phones.md`
      - `/home/max/Документы/ак2/живой_тест_номеров/ak2_cosmetology_01_kosmetologi_chat_full_history_phones.txt`
      - `/home/max/Документы/ак2/живой_тест_номеров/ak2_cosmetology_01_kosmetologi_chat_full_history_phones.json`
      - `/home/max/.site-control-kit/telegram_workspace/runs/20260602T110512Z/{summary.json,artifacts.json,events.jsonl}`
      - `/home/max/.site-control-kit/telegram_workspace/logs/export_run_20260602T110512Z.log`
  - `КОСМЕТОЛОГИЯ ЧАТ`:
    - `status=done`
    - `phones_found=3`
  ...
  - `Форум Косметология | Дерматология`:
    ...
    - итог сохранён как `status=partial`
    - `phones_found=2`
    - `history_messages_scanned=2211`
    - run: `20260602T113456Z`
    - partial artifacts сохранены:
      - `/home/max/Документы/ак2/живой_тест_номеров/ak2_cosmetology_04_chatkosmetologa_full_history_phones.md`
      - `/home/max/Документы/ак2/живой_тест_номеров/ak2_cosmetology_04_chatkosmetologa_full_history_phones.txt`
      - `/home/max/Документы/ак2/живой_тест_номеров/ak2_cosmetology_04_chatkosmetologa_full_history_phones.json`
  - `Косметологи Чат | Сообщество Профессионалов`:
    - не запускался после остановки batch
- Общая batch-сводка:
...
- Практический вывод:
  - full-history `public_phones` path на реальном `AK2` уже подтверждён живыми артефактами, а не только quick-check smoke;
  - следующий шаг теперь не новый refactor, а допройти 4-й чат без остановки и затем 5-й чат;
  - после code-fix нужен отдельный короткий confirm-run, что искусственно большой `TELEGRAM_TDATA_LIST_TIMEOUT_SEC` больше не нужен.

### Обновление 2026-06-02 (Отдельная кнопка `Сбор открытых номеров`)
- Добавлен второй основной Telegram export action рядом с `Собрать @username`: `Сбор открытых номеров`.
- Границы V1:
...
- Что изменено:
  ...
  - [scripts/telegram_gui/backend.py](../scripts/telegram_gui/backend.py), [scripts/telegram_gui/models.py](../scripts/telegram_gui/models.py), [scripts/telegram_gui/ui/window.py](../scripts/telegram_gui/ui/window.py) и [scripts/telegram_gui/ui/panels.py](../scripts/telegram_gui/ui/panels.py) теперь различают `operation_kind=usernames|public_phones`, ведут phone-aware history/progress/repeat-last-run и не смешивают артефакты двух режимов;
  - в phone TXT contract каждая строка теперь содержит `username`, `full_name`, `phone` в tab-separated виде.
- Verify:
  - targeted tests:
  ...
  - full suite:
    - `PYTHONPATH="$PWD" python3 -m unittest discover -s tests -p 'test_*.py'` -> `317 tests OK`
  - `git diff --check` -> OK
- Live smoke:
  - старый candidate `/home/max/telegram-api-collector/tdata_import/tdata` сейчас неавторизован;
  - рабочий candidate найден в `/home/max/site-control-kit/TG_CONTACT/4/tdata-003/tdata`;
  - интеграционный quick-check через новый backend route на чате `-1002465948544` создал:
    ...
    - `/home/max/.site-control-kit/telegram_workspace/runs/20260602T081357Z/summary.json`
    - `/home/max/.site-control-kit/telegram_workspace/runs/20260602T081357Z/artifacts.json`
    - `/home/max/.site-control-kit/telegram_workspace/runs/20260602T081357Z/events.jsonl`
  - результат smoke: `status=done`, `phones_found=1`, `history_messages=400`.
  - в этом же live smoke найден и сразу устранён false positive:
    - `t.me/c/2465948544/...` ошибочно считался телефоном;
    - после ужесточения regex повторный smoke `/tmp/site-control-live-public-phones-test-2_phones.json` дал `phones_found=0`.
- Практический вывод:
  - новый phone-flow уже не является только unit-level change set;
  - remaining next step, если понадобится, это ручная GTK operator acceptance на том же `Primary tdata`, а не новый code refactor.
...
- Среда:
  - `C:\site-control-kit-win-smoke`
  - `Windows 10 Pro`
  - `PowerShell 5.1.26100.8115`
  - `Python 3.14.0`
- Verify:
  - `scripts\start_hub.cmd` поднял hub без traceback;
  - `python -m webcontrol --help`, `python -m webcontrol browser --help`, `python -m webcontrol runtime-env --format json --no-create` -> OK;
...
- Практический вывод:
  ...
  - следующий безопасный шаг — использовать этот rerun как baseline и не расширять scope в новый Telegram feature-cycle.
...
- Выполнен documentation/handoff pass без новых runtime/browser/Telegram code changes:
  ...
  - `README.md` и `docs/INSTALL_OTHER_DEVICES_RU.md` теперь явно включают `telegram-username-collector` в Windows checklist и ведут в этот runbook;
  - desktop prompt `/home/max/Рабочий стол/промт_тестирование_пуша_юзеров_на_винде.txt` переписан, чтобы следующий Windows-агент проверял только узкий smoke по wrappers/runtime/fast-fail contract.
- Verify:
  - `python3 -m webcontrol --help` -> OK
  - `python3 -m webcontrol browser --help` -> OK
  ...
  - `git diff --check` -> OK
- Практический вывод:
  - documentation ambiguity вокруг Windows smoke снята;
  - широкий verify-контур и Linux-only GTK steps теперь явно выведены за scope этого handoff;
  - главный remaining gap не в docs, а в отсутствии живого Windows run с capture stdout/stderr и runtime-dir evidence.

### Обновление 2026-05-10 (Windows Core Smoke Dry Run + Draft v1 Release Checklist)
- Выполнен release-confidence проход без новых code/runtime изменений:
  - exact Windows checklist локально заменить было нельзя, потому что текущий host — Linux и в системе нет `wine`, `cmd.exe`, `pwsh`;
  - вместо этого прогнан весь обязательный verify-контур и безопасный Linux-эквивалент browser-side smoke.
- Verify:
  ...
  - browser-side smoke:
    ...
    - `bash scripts/start_hub.sh` -> hub поднялся
    - `python3 -m webcontrol health` -> OK
    - `./browser.sh status` -> OK
    - `./browser.sh tabs` -> OK
    - `python3 -m webcontrol clients` -> OK, но клиенты в state остаются `is_online=false`
    - после smoke hub был остановлен
  - Windows launcher fast-fail:
    - `python3 -m unittest tests.test_telegram_username_collector_launcher` -> `3 tests OK`
    - `scripts/telegram_username_collector_launcher.py` по-прежнему печатает controlled Windows-only fast-fail вместо traceback.
- Практический вывод:
  - runtime/wrapper contract подтверждён ещё одним проходом;
  - exact `Windows core smoke checklist` всё ещё остаётся открытым именно как live Windows machine step;
  - на текущей машине `runtime-env --no-create` резолвит adopted legacy runtime `~/.site-control-kit`, поэтому пункт про fresh checkout auto-create runtime dirs здесь не считается закрытым;
  - собран короткий draft `v1 release checklist`:
    - install
    - runtime-env
    - hub start/health
    - browser wrappers
  ...
  - локальные `.codex`, `TG_CONTACT/` и modified `artifacts/telegram_exports/INDEX.md` по-прежнему вне publish-checkpoint.
...
- Закрыт третий production-hardening пакет без смены Telegram export semantics:
  ...
  - shared globals `app.py` теперь зеркалятся в owner-модули, поэтому старый import/test contract не сломан;
  - implicit legacy collector home-default убран из GUI runtime и `bootstrap_telegram_workstation.sh`.
- Verify после фикса:
  - `python3 -m unittest discover -s tests -p 'test_*.py'` -> `294 tests OK`
  - `python3 -m webcontrol --help` -> OK
  - `python3 -m webcontrol browser --help` -> OK
  ...
  - clean install smoke:
    ...
    - `pip install -e .` -> OK
    - `sitectl --help` -> OK
    - `telegram-username-collector` -> controlled GTK fast-fail message вместо traceback
  - live GTK startup smoke на `DISPLAY=:0` снова подтвердил реальное окно `Telegram Username Collector`
  - `git diff --check` -> clean
- Практический вывод:
  - extraction backend/window уже landed как реальная структура, а не как alias-заготовка;
  - hidden fallback на `/home/max/telegram-api-collector` больше не участвует в production defaults;
  - следующий безопасный шаг теперь уже не ещё один GUI split, а Windows core smoke и короткий release checklist;
  - exact Windows verify-набор уже зафиксирован в `README.md` и `docs/INSTALL_OTHER_DEVICES_RU.md`, и следующий агент должен пройти его перед новым feature-cycle.

### Обновление 2026-05-09 (Production Hardening Change Set 2: Logging + Launcher + Install Story)
- Закрыт второй production-hardening пакет без изменения Telegram export semantics:
  - `var/` добавлен в `.gitignore`;
  - `PyGObject` удалён из `requirements.txt` и `pyproject.toml`: GTK GUI теперь документирован как system dependency Linux;
  - новый launcher `telegram-username-collector` теперь даёт controlled fast-fail на Windows и в Python-окружении без GTK bindings;
  ...
  - Telegram GUI теперь пишет per-run machine-readable sidecars:
...
- Verify после фикса:
  ...
  - `python3 scripts/export_telegram_members_non_pii.py --help` -> OK
  - `bash scripts/bootstrap_telegram_workstation.sh --doctor` -> OK
  - `bash scripts/start_hub.sh` -> hub поднялся; `python3 -m webcontrol health` -> OK; `python3 -m webcontrol state` -> OK
  - clean install smoke:
    - `pip install -r requirements.txt` -> OK
    - `pip install -e .` -> OK
    - `sitectl --help` -> OK
    - `telegram-username-collector` -> controlled GTK fast-fail message вместо import/build traceback
  - live GTK startup smoke на `DISPLAY=:0` снова подтвердил реальное окно `Telegram Username Collector`
- Практический вывод:
  - install story больше не ломается на `PyGObject` при чистом `pip install`
  - runtime logging и Telegram run diagnostics теперь пригодны для сопровождения
  - следующий безопасный шаг уже не baseline/bootstrap, а минимальный extraction backend/window orchestration из `scripts/telegram_gui/app.py`

### Обновление 2026-05-09 (Production Hardening Change Set 1: Runtime/Config Baseline)
- Закрыт первый production-hardening пакет без смены Telegram export semantics:
  ...
  - если уже существует legacy runtime `~/.site-control-kit`, проект больше не переносит его автоматически, а создаёт compatibility pointer `.site-control-kit/local.yaml`;
  - core entrypoints и wrappers больше не используют `local-bridge-quickstart-2026` как рабочий fallback token;
  - `scripts/export_telegram_members_non_pii.py` больше не держит `/home/max/...` output path как default;
  - `scripts/reload_bridge_extension.sh` теперь сначала честно проверяет доступность хаба и даёт понятную ошибку, а не traceback.
- Verify после фикса:
  ...
  - live GTK startup smoke:
    ...
    - после завершения `xwininfo` уже не видит этот title.
- Практический runtime-результат:
  - на текущей машине compatibility mode реально adopted existing legacy runtime:
    - `.site-control-kit/local.yaml` указывает на `~/.site-control-kit`;
    - `.site-control-kit/generated_token.txt` создан локально и используется wrappers/CLI;
    - `bootstrap --doctor` и GUI остались на legacy workspace без auto-move.
- Практический вывод:
  - install/runtime story уже не завязан жёстко на `/home/max/...` и больше не требует quickstart token в shell wrappers;
  - следующий безопасный этап теперь уже про unified logging/error-reporting и минимальный extraction runtime/logging orchestration из `scripts/telegram_gui/app.py`.

### Обновление 2026-05-09 (TG_CONTACT 4 `ROST FARMA` Stability And Live Export Cycle)
- Новый strongest target на `TG_CONTACT 4` уже подтверждён живым прогоном:
  - нового кода в репозитории в этом цикле не потребовалось;
  - основной `Primary tdata` path уже отработал end-to-end без нового blocker-а;
  - backend direct probe подтвердил live row:
    - `Чат ROST FARMA`
...
- Preflight перед прогоном был чистым:
  ...
  - browser hub поднимался только как secondary contour:
    - `bash scripts/start_hub.sh` -> OK
    - `./browser.sh status` -> OK
    - `./browser.sh tabs` -> OK
    - browser clients остались stale/offline, но это не стало blocker-ом для `tdata-history-authors`
...
- Практический вывод:
  ...
  - browser hub/offline clients сейчас не надо путать с blocker-ом Telegram export path;
  - следующий шаг теперь уже брать ещё один productive target на `TG_CONTACT 4`, не смешивая это с secure token / legacy cleanup.
