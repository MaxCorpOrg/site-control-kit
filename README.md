...
- если задан высокий `CHAT_MIN_MEMBERS`, chat-экспорт переходит в discovery-first режим: сначала быстрее добирает новых авторов, а deep запускает реже, чтобы не сжечь весь runtime на первых шагах;
