...
### Сервисные границы и быстрая доставка команд

- HTTP-маршруты браузерного контура переведены на `HubServices`;
- выделены сервисы команд, клиентов, сессий и состояния;
- добавлен совместимый долгий запрос команд с ожиданием до 25 секунд;
- транспорт расширения вынесен в отдельный модуль;
- сквозной тест в реальном Chrome подтвердил медианную доставку за `13,62`
  мс без потерь и повторных опасных действий;
- сохранена воспроизводимая контрольная точка
  [`docs/checkpoints/CHECKPOINT_2026-07-24_SERVICES_LONG_POLL_RU.md`](docs/checkpoints/CHECKPOINT_2026-07-24_SERVICES_LONG_POLL_RU.md).
