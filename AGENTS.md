...
1. `AUTOPILOT.yaml` — repo-local режим работы без лишних подтверждений и единый verify-контур.
2. `BROWSER_QUICKSTART.md` — короткий путь запуска и базовые команды.
3. `docs/AI_MAINTAINER_GUIDE.md` — как агенту использовать и развивать инструмент.
4. `docs/API.md` — протокол, типы команд, контракт результата.
5. `docs/ARCHITECTURE.md` — поток команд, роли компонентов, маршрутизация.
6. `docs/EXTENSION.md` — где реализованы background- и DOM-команды.
