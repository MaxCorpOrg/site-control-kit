...
import json
import traceback
from tkinter import END, BOTH, LEFT, RIGHT, VERTICAL, Y, Listbox, StringVar, Tk
...
class ToolPlatformPanel(Tk):
    def __init__(self, registry_path: str) -> None:
        ...
        self.tool_list = Listbox(self, exportselection=False)
        self.action_list = Listbox(self, exportselection=False)
        self.details = None
        self._build_ui()
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(header, text="Reload", command=self._reload_catalog).pack(side=RIGHT)
        ...
        actions_frame.pack(fill=BOTH, expand=False, pady=(12, 0))
        ttk.Label(actions_frame, text="Actions").pack(anchor="w")
        action_body = ttk.Frame(actions_frame)
        action_body.pack(fill=BOTH, expand=True, pady=(8, 0))
        self.action_list.pack(in_=action_body, side=LEFT, fill=BOTH, expand=True)
        self.action_list.bind("<<ListboxSelect>>", self._on_action_select)
        action_buttons = ttk.Frame(action_body)
        action_buttons.pack(side=RIGHT, fill=Y, padx=(12, 0))
        ttk.Button(action_buttons, text="Run Action", command=self._run_action).pack(fill="x")
        ttk.Button(
            action_buttons,
            text="Preview Action",
    ...
    def _reload_catalog(self, initial: bool = False) -> None:
        ...
        for tool in self.catalog.tools:
            ...
            self.tool_list.insert(END, f"{tool.display_name} [{standalone}]")
    ...
    def _show_tool(self, tool: ToolManifest) -> None:
        ...
        for key, value in rows:
            if value:
                self.details.insert("", END, text=key, values=(value,))
        if tool.docs:
            parent = self.details.insert("", END, text="docs", values=("",))
            for doc in tool.docs:
                self.details.insert(parent, END, text=doc.label, values=(str(doc.path),))
        if tool.artifacts:
            parent = self.details.insert("", END, text="artifacts", values=("",))
            for key, value in tool.artifacts.items():
                self.details.insert(parent, END, text=key, values=(value,))
        self.action_list.delete(0, END)
        for action in tool.actions:
            self.action_list.insert(END, f"{action.label} [{action.action_id}]")
