...
def default_agent_state() -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "current_priority": "Linux rootful release closeout для Telegram Control Center уже закрыт; следующий product priority — Windows installer artifact на Wine/Inno-capable runner, а локальная Linux reinstall state теперь только operator housekeeping.",
        "active_risks": [
            ...
            "Локальная установленная Linux-копия на этой машине сейчас снята после rootful uninstall acceptance; если GUI снова нужен локально, оператор должен вручную переустановить `0.1.1`.",
        ...
        "last_verified_artifacts": {
            ...
            "linux_rootful_install_acceptance": "dpkg -s telegram-control-center -> Version 0.1.1, self-test OK from /opt/site-control-kit/app",
            "linux_rootful_uninstall_acceptance": "dpkg -s telegram-control-center -> deinstall ok config-files; dpkg -L -> no installed files",
            "linux_desktop_shortcut": "/home/max/Рабочий стол/telegram-control-center.desktop",
        ...
        "next_recommended_step": "Product-wise дальше закрыть Windows artifact на машине/runner с Wine + Inno; если локально снова нужен Linux GUI, оператору отдельно переустановить `packaging/dist/linux/telegram-control-center_0.1.1_all.deb`.",
        "updated_at": "",
    }
